...
try {
    ...
    // 從解析過後的資料中提取特定屬性值
    $sta_id = $input['sta_id'];
    $sta_email = $input['sta_email'];
    $sta_acc = $input['sta_acc'];
    $sta_psw = $input['sta_psw'];

    // 準備 SQL 更新語句，請根據您的數據庫實際情況進行調整
    $sql = "UPDATE staff SET  sta_email = ?, sta_acc = ?, sta_psw = ? WHERE sta_id = ?";

    // 預處理 SQL 語句
    $stmt = $pdo->prepare($sql);

    // 綁定參數到預處理語句
    $stmt->bindParam(1, $sta_email);
    $stmt->bindParam(2, $sta_acc);
    $stmt->bindParam(3, $sta_psw);
    $stmt->bindParam(4, $sta_id);
